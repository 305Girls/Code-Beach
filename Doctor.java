...
public class Doctor extends Account
{
	...
	public Doctor(String dId)
	{
	    ...
	    String sql1="select dId,name,age,gender,dpmId from Doctor "
	    		+ "where dId='"+dId+"'";
		String sql2="select B.name,A.pId,A.qNumber,A.isVisit "
    ...
    public Patient passPatient()//过号
    {
    	...
    	else if(p.getIsVisit()=='3')//预约病人，改appTime为当前系统时间，改状态位为0无状态
    	{
    		Date date=new Date(System.currentTimeMillis()); 
    		DateFormat format=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    		String timeNow=format.format(date);//当前系统时间
    		String sql="update PatientHsp set isVisit=0 "
    				+ "and appTime ="+timeNow+" where pId='"+p.getpId()+"'";
    		OPDB.excuteIUD(sql);
